...
def test_api_keys():
    ...
    r = requests.post(
        f"{BASE_URL}/admin/api-keys?token={ADMIN_TOKEN}", json=create_payload
    )
    ...
    r2 = requests.post(
        f"{BASE_URL}/api/zapytaj", json={"pytanie": "jak napisać podanie"}
    )
    ...
    r3 = requests.post(
        f"{BASE_URL}/api/zapytaj", json={"pytanie": "co to jest USOS"}, headers=headers
    )
    ...
    r5 = requests.get(f"{BASE_URL}/admin/api-keys?token={ADMIN_TOKEN}")
    print(f"Liczba kluczy w systemie: {len(r5.json())}")

    print("\n--- 6. Revokacja klucza ---")
    r6 = requests.post(f"{BASE_URL}/admin/api-keys/{key_id}/revoke?token={ADMIN_TOKEN}")
    print(f"Status revokacji: {r6.status_code} - {r6.json()}")

    print("\n--- 7. Wywołanie /api/zapytaj na odwołanym kluczu ---")
    r7 = requests.post(
        f"{BASE_URL}/api/zapytaj", json={"pytanie": "test"}, headers=headers
    )
