...
def run_test():
    ...
    for i in range(max_retries):
        try:
            ...
            response = requests.post(
                url, 
                json={"pytanie": "ile razy mozna zdawac egzamin?"},
                timeout=10
            )
            if response.status_code == 200:
                data = response.json()
                if "odpowiedz" in data or "wstep" in data:
                    print(f"[OK] Proba {i+1}: Serwer odpowiedzial poprawnie!", flush=True)
                    success = True
                    break
            else:
                print(f"Proba {i+1}/{max_retries}: Serwer zwrocil status {response.status_code}", flush=True)
        except Exception as e:
            print(f"Proba {i+1}/{max_retries}: Serwer jeszcze nie gotowy...", flush=True)
    ...
    if success:
        ...
        print("--- BLAD: Smoke Test NIEZALICZONY (serwer nie odpowiedzial poprawnie) ---", flush=True)
        sys.exit(1)
